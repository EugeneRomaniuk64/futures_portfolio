...
def to_simple(returns: float) -> float:
...
if __name__ == "__main__":
    ...
    weights = [1/len(tickers_list) for i in range(len(tickers_list))]
    investment = 5_000_000_000
    pnl: pd.Series = get_pnl(df, weights, investment)

    VaR, ES, sim_pnl, total_pnl = get_risk(pnl.mean(), pnl.std(), 21, 0.91, get_sim_returns=True, df=4)

    create_hist_plot(total_pnl, title="Risk Metrics over 21 days", lines=(VaR, ES), lines_labels=("VaR", "Expected Shortfall"), line_colors=("yellow", "red"))
    ...
    '''
    start_date = dt.datetime(2008, 9, 15)
    end_date = dt.datetime(2008, 10, 10)
    scenario_df = df[df.index >= start_date and df.index <= end_date]
    scenario_pnl = get_pnl(scenario_df, weights, investment)

    VaR, ES, sim_pnl, total_pnl = get_risk(scenario_pnl.mean(), scenario_pnl.std(), 30, df=4, get_sim_returns=True)

    create_hist_plot(total_pnl, title="Risk Metrics over 30 days", lines=(VaR, ES), lines_labels=("VaR", "Expected Shortfall"), line_colors=("yellow", "red"))