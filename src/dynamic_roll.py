...
class DynamicRoll:
    def import_futures_data_from_csv(self, file_name: str) -> pd.DataFrame:
        try:
            ...
            df.columns = ['TICKER', 'PRICE', 'EXPIRY_DATE', 'VOLUME', 'OPEN_INTEREST']
    ...
    def dynamic_roll_optimizer(self, df_curve: pd.DataFrame) -> dict:
        ...
        T_front = front_contract['EXPIRY_DATE']
        ...
        for i in range(1, len(df_curve)):
            ...
            T_roll_in = roll_in_contract['EXPIRY_DATE']
            

            time_diff_days = (T_roll_in - T_front).days
            if time_diff_days <= 0: continue
            
            Delta_t = time_diff_days / 365.25
...
if __name__ == "__main__":
    ...
    curve_data = roller.import_futures_data_from_csv("./data/GC.csv")
    ...
    display_cols = ['RollInTicker', 'ImpliedYield', 'RollInPrice', 'DeltaT_Years']
